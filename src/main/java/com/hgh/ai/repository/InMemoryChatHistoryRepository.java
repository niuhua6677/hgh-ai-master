...
import com.hgh.ai.entity.po.Msg;
...
public class InMemoryChatHistoryRepository implements ChatHistoryRepository {
    ...
    /**
     * 将给定的聊天ID保存到指定类型的聊天历史记录中
     * 如果该类型尚未在聊天历史记录中创建，则会为其创建一个新的列表
     * 如果聊天ID已经存在于该类型的历史记录中，则不执行任何操作
     */
    @Override
    public void save(String type, String chatId) {
        // 使用computeIfAbsent方法优化代码，确保如果类型不存在时创建一个新的ArrayList，避免了显式的containsKey检查
        List<String> chatIds = chatHistory.computeIfAbsent(type, k -> new ArrayList<>());
        // 检查聊天ID是否已经存在于列表中，以避免重复保存
        if (chatIds.contains(chatId)) {
            return;
        }
        // 将聊天ID添加到对应聊天类型的历史记录列表中
        chatIds.add(chatId);
    }
